...
AABCharacter::AABCharacter()
{
	...
	static ConstructorHelpers::FClassFinder<UAnimInstance> WARRIOR_ANIM(
		TEXT("/Game/Animations/WarriorAnimBlueprint.WarriorAnimBlueprint_C"));
	if (WARRIOR_ANIM.Succeeded())
	{
		GetMesh()->SetAnimInstanceClass(WARRIOR_ANIM.Class);
	}

	SetControlMode(EControlMode::DIABIO);

	ArmLengthSpeed = 3.0f;
	ArmRotationSpeed = 10.0f;
	// default jump velo = 420.0f
...
void AABCharacter::Tick(float DeltaTime)
{
	...
	case EControlMode::DIABIO:
		...
		{
			// ¹«½¼ ÄÚµå?
			GetController()->SetControlRotation(FRotationMatrix::
				MakeFromX(DirectionToMove).Rotator());
...
void AABCharacter::UpDown(float NewAxisValue)
{
	...
	{
	case EControlMode::GTA:
		AddMovementInput(FRotationMatrix(FRotator(0.0f, GetControlRotation().Yaw, 0.0f)).
			GetUnitAxis(EAxis::X), NewAxisValue);
		break;
	case EControlMode::DIABIO:
...
void AABCharacter::LeftRight(float NewAxisValue)
{
	...
	case EControlMode::GTA:
		AddMovementInput(FRotationMatrix(FRotator(0.0f, GetControlRotation().Yaw, 0.0f)).
			GetUnitAxis(EAxis::Y), NewAxisValue);
		break;
	case EControlMode::DIABIO:
